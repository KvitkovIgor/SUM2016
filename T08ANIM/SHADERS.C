 ...
 * RETURNS: None.
 */
static VOID IK3_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("IK3{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
} /* End of 'IK3_RndShaderLoadTextFile' function */
 ...
 */
static CHAR * IK3_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
...
UINT IK3_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      IK3_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = IK3_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      IK3_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'IK3_RndShaderLoad' function */
...
VOID IK3_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'IK3_RndShaderFree' function */
