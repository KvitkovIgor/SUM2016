 ...
 * RETURNS: None.
 */
VOID LoadSphere( char *loadfileName )
{
  ...
  hBm = LoadImage(NULL, loadfileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
VOID DrawSphere( HDC hDC, INT Xc, INT Yc, INT R )
{
  ...
  for (i = 0; i < M; i++)
  {
    ...
    for (j = 0; j < N; j++)
    {
      phi = j * 2 * PI / (N - 1) + clock() / 1000.0;
      G[i][j].X =  R * sin(theta) * cos(phi);
      G[i][j].Y =  R * sin(theta) * sin(phi);
      G[i][j].Z =  R * cos(theta); 
    
      G[i][j] = Rotate(G[i][j], A, 30 * sin(clock() / 1000.0));
    }                          
  }
  for (i = 0; i < M; i++)
    for (j = 0; j < N; j++)
